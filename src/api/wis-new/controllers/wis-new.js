...
module.exports = createCoreController("api::wis-new.wis-new", ({ strapi }) => ({
  async find(ctx) {
  ...
  async findBlog(ctx) {
    ...
    return await strapi.entityService.findMany("api::wis-new.wis-new", {
      ...ctx.query,
      filters,
  ...
  async findOne(ctx) {
    return await strapi.config.functions.controllerOverride.findOneBySlug(
      ctx,
      "api::wis-new.wis-new",
      ctx.query.populate || strapi.config["population-schemas"].collections.news
    );
  },
}));
